...
5. بررسی وجود مخزن روی گیت‌هاب؛ **تصحیح خودکار نام کاربری از روی توکن** و **ساخت خودکار مخزن** با تأیید شما
6. **هم‌سان‌سازی تاریخچه با مخزن گیت و سپس کامیت** — پایه‌ی تاریخچه روی نوک مخزن تنظیم می‌شود،
   پس ارسال همیشه سریع (fast-forward) است: **بدون تعارض، بدون جایگزینی اجباری، بدون پاک‌شدن تاریخچه**.
   فایل‌های شما جایگزین می‌شوند و فایل‌هایی که فقط روی گیت هستند حذف نمی‌شوند.
7. ارسال به گیت‌هاب + تلاش دوباره در صورت رد شدن + جایگزینی اجباری فقط با تأیید شما
8. ساخت و ارسال تگ نسخه (اگر تگ روی گیت به کامیت قدیمی اشاره کند، خودکار جابه‌جا می‌شود)
   + پیشنهاد آپلود فایل بسته در بخش Releases

نکات مهم:

- **بار اول** توکن و نام مخزن را می‌پرسد؛ از آن به بعد اجرا کاملاً خودکار است.
- **تعارض و جایگزینی اجباری تمام شد**: از نسخه‌ی جاری، تاریخچه پیش از کامیت هم‌سو می‌شود؛
  پس دیگر دیوار متن `CONFLICT` و `forced update` نمی‌بینید و تاریخچه‌ی مخزن گیت دست‌نخورده می‌ماند.
...
| تگ روی گیت به کامیت قدیمی اشاره می‌کند | تگ فقط یک‌بار ساخته شده و بعد از کامیت‌های جدید جابه‌جا نشده | نسخه‌ی جاری `git-push.bat` خودکار تگ را به کامیت فعلی منتقل و به‌روزرسانی می‌کند |
